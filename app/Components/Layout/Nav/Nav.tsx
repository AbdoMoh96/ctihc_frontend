            ...
            <div className={`h-20 font-roboto rtl:font-cairo transition-all bg-zinc-800 text-white flex justify-center duration-500 ease-in-out ${navState}`}>
                  <Container className='flex justify-between items-center'>
                   ...
                   <div className={`fixed transition-all duration-500 ease-in-out top-20 z-500 w-full text-white bg-zinc-800 flex pt-4 lg:pt-0 gap-6 flex-col justify-start items-center lg:justify-between lg:transition-none lg:w-auto lg:flex-row lg:gap-7 lg:bg-transparent h-full lg:text-white lg:mt-0 lg:static ${showMenu ? 'right-0' : '-right-full' }`}>
                          <NavLink href='/' replace={true} onClick={() => setShowMenu(false)} text={'__translate_home'}/>
                          <NavLink href='/about' replace={true} onClick={() => setShowMenu(false)} text={'__translate_about_us'}/>
                          <NavLink href='/reports' replace={true} onClick={() => setShowMenu(false)} text={'__translate_reports'}/>
                          <NavLink href='/group-structure' replace={true} onClick={() => setShowMenu(false)} text={'__translate_group_structure'}/>
                          <NavLink href='/news' replace={true} onClick={() => setShowMenu(false)} text={'__translate_news'}/>
                          <NavLink href='/partners' replace={true} onClick={() => setShowMenu(false)} text={'__translate_partners'}/>
                          <NavLink href='/contact' replace={true} onClick={() => setShowMenu(false)} text={'__translate_contact_us'}/>
                       {/* {lang == 'en' &&
