...
const DropDown : React.FC<propTypes> = ({children, title}) => {
    ...
    return <div
        ...
        {title}
        <div
            className={`lg:invisible text-nowrap ${showDropDown ? 'max-h-60' : 'max-h-0'} lg:max-h-max overflow-hidden w-full bg-white lg:drop-shadow-lg lg:rounded transition-all duration-700 lg:duration-200 ease-in-out lg:absolute mt-3 z-10 top-full lg:w-auto lg:py-2 text-black pl-5 pr-2 flex flex-col items-end gap-2 right-0 lg:opacity-0 group-hover:visible lg:group-hover:mt-1 group-hover:opacity-100`}>
            {children}
        </div>
    </div>
}
